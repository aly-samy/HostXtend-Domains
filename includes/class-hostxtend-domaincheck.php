...
class HOSTXTEND_DOMAIN_CHECK {
	...
	public function init() {
		...
		$domaincheck = new HOSTXTEND_DOMAIN_CHECK_Domaincheck( $this->options );
	...
	public function enqueue_scripts() {
		...
		if ( file_exists( get_stylesheet_directory() . '/hostxtend-domain-check/assets/js/domaincheck.js' ) )
			$js_file_path = get_stylesheet_directory_uri() . '/hostxtend-domain-check/assets/js/domaincheck.js';

		// just register the scripts, enqueue in shortcode
		wp_register_script(
			'hostxtend-domaincheck',
			$js_file_path,
			array( 'jquery' ),
	...
	public function shortcode( $atts = [], $content = NULL, $tag = '' ) {
		...
		if ( 'results' == $atts['output_type'] )
			return '<div id="dc-result-' . $id . '" class="hostxtend-dc"></div>';
		...
		$js = 
			"jQuery( function( $ ) {\n".
			"  $( '#hostxtend-dc-" . $id . "' ).HOSTXTEND_DOMAIN_CHECK( {\n";
		$js .=
			"    id: '" . $id . "',\n".
		...
		if ( function_exists( 'wp_is_block_theme' ) && wp_is_block_theme() ) {
			...
			$html = '<script src="' . plugins_url( 'assets/js/domaincheck.js', dirname( __FILE__ ) ) . '" id="hostxtend-domaincheck-js"></script>' . PHP_EOL;
			if ( boolval( $atts['addjs'] ) )
				$html .= '<script id="hostxtend-domaincheck-js-after">' . $js . '</script>' . PHP_EOL;
			...
			wp_enqueue_script( 'hostxtend-domaincheck' );
			if ( boolval( $atts['addjs'] ) )
				wp_add_inline_script( 'hostxtend-domaincheck', $js );
		}
		...
		return '<div id="hostxtend-dc-' . $id . '" class="hostxtend-dc"></div>';
	}
